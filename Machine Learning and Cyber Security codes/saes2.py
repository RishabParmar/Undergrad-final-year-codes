...
def subNibble(nib, box):
    # print("Subnibble :", bin(int(nib, 16))[2:].zfill(4))
...
def genT(oddw, rc):
    # print("The odd[0] :", oddw[0])
    # print("The odd[1] :", oddw[1])
...
def keygen(key):
    ...
    print("Value :", hex(int(key.replace(" ", ''), 2))[2:])
    k = hex(int(key.replace(' ', ''), 2))[2:]
    print("The value of the hex in decimal :", int(k, 16))
    # print k
    # 24
    w[0] = k[:2]
    # 75
...
def mult(num, h):
    ...
    if num != 9:
        if val >= 64:
            val ^= 76
        if val >= 32:
            print("bruh value :", val)
            val ^= 38
            print("bruh value after:", val)
        if val >= 16:
            val ^= 19
...
def main():
    subk = keygen(key)
    print("The keys generated are :", subk)
